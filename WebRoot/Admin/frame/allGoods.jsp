...
<link href="../css/style.css" rel="stylesheet" type="text/css" />
	...
	<script type="text/javascript">
		function test(){
			var a = document.getElementById("permission").value;
			alert(a);
		}
	</script>
								...
								<s:iterator value="goodsList" id="g">
									<tr class="alt-row">
										...
										<td>
											...
											<s:if test="%{#g.gdStatus.equals(\"1\")}">
												审核中..
											</s:if>
											<s:elseif test="%{#g.gdStatus.equals(\"2\")}">
												销售中..
											</s:elseif>
											<s:elseif test="%{#g.gdStatus.equals(\"3\")}">
												已下架
											</s:elseif>
											<s:else>
												<s:property value="#g.gdStatus"/>
											</s:else>
										</td>
										<td>
											<!-- Icons -->
...
<form><table>
			...
			<tr id="line_state"><td class="xiun"><strong>状  态</strong></td><td><div class="input">
						<input name='state' type='radio' value='已结束'>已结束 <input name='state' type='radio' value='进行中'>进行中 
					<div class="none"><span></span></div></div></td></tr>
