$(function(){
	...
	$(".act").click(function(){/*
			...
			$myAlert.find("input").eq(2).attr("value",$before.children('td').eq(4).text());
			$myAlert.find("input").eq(3).attr("value",$before.children('td').eq(5).text());
			$myAlert.find("input").eq(4).attr("value",$before.children('td').eq(6).text());
			$myAlert.find("textarea").eq(0).attr("value",$before.children('td').eq(7).text());
			$myAlert.find("input").eq(5).attr("value",$before.children('td').eq(8).text());
			$myAlert.find("input").eq(6).attr("value",$before.children('td').eq(9).text());
			$lin = $before.children('td').eq(10).text();
					if($lin=="已付款"){
						$myAlert.find("input").eq(7).attr("checked","true");
					}
					if($lin=="未付款"){
						$myAlert.find("input").eq(8).attr("checked","true");
					}
	...
	$(".content-add").click(function(){/*
			...
			$myAlert.find("input").eq(6).attr("value","");
			$myAlert.find("input").eq(7).attr("checked","true");
            var iX, iY;
            $("#alert input").mouseover(function(){
            	tableshang1=1;
