...
$(document).ready(function(){
	...
	$('.check-all-all').click(
			function(){
				$(this).parent().parent().parent().parent().parent().parent().find("input[type='checkbox']").attr('checked', $(this).is(':checked'));   
			}
		);
	$('.dsee').click(function(){
		$(this).parent().parent().next().fadeIn(600);
	})
	$('.dshouqi').click(function(){
...
$(function(){
			...
			$(".highlight_tip span").mouseover(function(){
				...
				var $nowNum = $(".highlight_tip span.current").attr("id")[3];
				var $distance = ($num-$nowNum)*700;
				if(!$v_show.is(":animated")){
				$v_show.animate({left:'-='+$distance},"600");
				page+=($num-$nowNum);
					$(".highlight_tip span").eq(page-1).addClass("current").siblings().removeClass("current");
				//alert($distance);
				}
			...
			/*控制左右按钮出现*/
			var $leftRightButton = $("#v_show .change_btn");
			var $focusPicture = $("#v_show .v_content");
			$focusPicture.hover(function(){
				$leftRightButton.show();
			},function(){
				$leftRightButton.hide();
			});
			$leftRightButton.hover(function(){
				$leftRightButton.show();
			});
/*end of --焦点图*/
